...
import config from '../../utils/signaling.config.json';

export const useClient = createClient(config?.appId);
export const useChannel = createChannel(config?.channelName);

const SignalingReactSDK = () => {
  ...
  return (
    <div style={{ display: 'flex', flex: 1, flexDirection: 'column', height: '90vh', margin: 8 }}>
      ...
      <div style={{ display: 'flex', margin: 'auto' }}>
        ...
        />
        <button disabled={!uid} style={btnStyle} onClick={isLoggedIn ? logout : login}>
...
export default SignalingReactSDK;
